...
group_merged_distributions = make_group_summaries(summary_bird);
...
[groups, ~, ii_group] = unique({summary_bird.group});
summary_bird_medians = get_summary_stats(summary_bird, statFunction=@median);
summary_bird_medians_all_stims = get_summary_stats(summary_all_stims, statFunction=@median);
...
for i_ftp = 1:length(fields_to_plot)
    ...
    for i_gr = 1:length(groups)
        group = groups{i_gr};
        group_medians = [summary_bird_medians(ii_group == i_gr).(field)];
...
[p_vals_all_stims] = get_stats_dm_pam(summary_bird_medians_all_stims, all_stim_fields);
[p_vals_calls] = get_stats_dm_pam(summary_bird_medians, call_only_fields);
...
save(fname, ...
    ...
    'p_vals_all_stims', ...
    'p_vals_calls', ...
    ...DISTRIBUTIONS BY BIRD
    'summary_bird', 'summary_all_stims', 'summary_bird_exclusions',...
    ...MEDIANS BY BIRD
    'summary_bird_medians',...
