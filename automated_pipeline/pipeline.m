...
if ~merge_files
    parameter_names{end+1} = 'filename';
end
...
if isfield(p.call_seg, "manual_filepath")
    manual_filepath = p.call_seg.manual_filepath;
    
    % clear call seg fields in p to assert manual labels used.
...
call_seg_data = s3_segment_calls( ...
    ...
    p.call_seg.q, ...
    ... # valid call timing
    p.call_seg.post_stim_call_window_ms, ...
    ... # overwrite all the other stuff if a path is described.
    ManualCallSegFilePath = manual_filepath ...
);
